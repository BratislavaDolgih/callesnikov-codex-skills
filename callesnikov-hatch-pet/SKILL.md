...
```text
Research a brand for Callesnikov Hatch Pet mascot creation.
...
```bash
SKILL_DIR="${CODEX_HOME:-$HOME/.codex}/skills/callesnikov-hatch-pet"
...
```text
Generate the Callesnikov Hatch Pet base image.
...
```text
Generate one Callesnikov Hatch Pet four-cardinal anchor strip.
...
```text
Generate one Callesnikov Hatch Pet row.
...
```text
Classify one required gaze axis in an unlabeled Callesnikov Hatch Pet A/B challenge.
...
```text
Visually QA one finalized Callesnikov Hatch Pet contact sheet.
